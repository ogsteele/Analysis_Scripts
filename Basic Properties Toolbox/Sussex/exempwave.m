% exempwave script for saving IStep exemplary figures. 
% note
[file, path] = uigetfile('*.*');
...
% plot waveform of current step read from second channel of clampfit
        ...
        fig_protocol = figure();; plot(x,pA_waveform*1e12,'linewidth',1,'color','black','HandleVisibility','off')
...
Time = S.array(:,1);
Waves = S.array(:,2:end);
fig_recbasic = figure(); plot(Time,Waves*1000,'Color', 'Black')
    xlabel('Time (s)')
...
if run == "Yes"
    saveas(fig_recbasic,'recbasic.emf')
    saveas(fig_protocol,'protocol.emf')
    % plot number of spikes 
        ...
        for i = 1:size(Waves,2)
            ...
            [pks{i},locs{i},w{i},p{i}] = findpeaks(Waves(20000:60000,i),...
                'MinPeakHeight',0,...
    ...
    disp('enter current value (pA) to display spike frequency at')
    prompt = {...
        'Enter current value (pA) to display spike frequency at:'};
    dlg_title = 'Display wave';
    num_lines = 1;
    def = {'300'};
    answer  = inputdlg(prompt,dlg_title,num_lines,def,'on');
    answer = str2double(answer);
    step = find(pA==300);
    fig_wavebasic = figure(); plot(S.array(:,1), S.array(:,step)*1000, 'Color','Black','LineWidth',1.5)
        xlabel('Time (s)')
        ylabel('Membrane Potential (mV)')
        box off; set(gcf,'color','white'); set(gca,'linewidth',2)
        ax = gca; yax = ax.YAxis; set(yax,'TickDirection','out')
        title('Exemplary Wave')
    % would you like to save this as an emf? 
    dlgTitle    = 'Save as emf?';
    dlgQuestion = 'Do you want to save a figure out of this trace?';
    emf = questdlg(dlgQuestion,dlgTitle,'Yes','No','No');
    if emf == "Yes"
        saveas(fig_wavebasic,'wavebasic.emf')
    else 
    end
    pause(2)
else
