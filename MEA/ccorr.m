...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Cross correlation from NeurAn toolbox     
% altered to plot 
...
lim=10e3;
if length(data1)>lim
    %r = 1 + (length(data1)-a).*rand(1,15000);
    data1=data1(1:lim);
    disp('ccorr: using only the first 10e3 spikes of data1!') 
end
if length(data2)>lim
    %r = 1 + (length(data2)-a).*rand(1,15000);
    data2=data2(1:lim);
    disp('ccorr: using only the first 10e3 spikes of data2!') 
end
...
if flag1 ~= flag2;
 error(['data1 and data2 should be the ' ...
...
if isempty(T);
    if flag
       T = [min(smp) max(smp)]; 
        ...
        T(1)=min([ min(data1) min(data2)] ); 
        T(2)=max([ max(data1) max(data2)] );
...
% figure out the bin density and grid...
  ...
  DT = T(2)-T(1);
  spcfac = 5;  
  % density of bins for kernel smoother (bins per sig)
...
%   different normalizations...    
    ...
    elseif abs(method) == 4  % make it a corr coeff?     
      %std_N(n,:)= length(data1)*length(data2)/Nbins;      
      ccorr(n,:) = (ccorr(n,:) - mn_N(n,:))./std_N(n,:);
    end
  end  
 %pause
  if abs(method) == 2  % shuffle cross-corr with other trials...
    shuffle_corr = 0;
    for n = 1:NT
      for nn=n+1:NT
	intervals = ...
	...
	%indx = find(intervals > smp(1) & intervals < smp(2));
        intervals = intervals(intervals > smp(1) & intervals < smp(2));
        if method > 0
...
if ~strcmp(plt,'n')
  if flag
    ...
    if method > 0   
      bar(tau,mu,plt(1),'EdgeColor','none', 'Barwidth',1);
    else
      plot(tau,mu,'r-');
    end
    line([0 0],get(gca,'ylim'),'color','k')
